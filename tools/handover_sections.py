...
STACK = [
    ...
    ["Scanned documents", "Tesseract OCR (Hebrew, Arabic + English) with PDFium", "Reads PDFs that are images rather than text, and the pictures inside Word files and slide decks whose content is images. Runs in our own container, so it costs no fees — only time."],
...
PAGES = [
    ...
    ["Knowledge base", "/knowledge", "Administrators",
     "A municipality administrator sees one library: their own. Any of that municipality's administrators may fill, edit or empty it, and nobody outside the municipality can read it. A system admin sees every library, switching between them at the top of the screen, and is the only one who adds to the shared library that all municipalities draw on. The shared library is deliberately not offered to municipality administrators as a place to browse — they cannot add to it, and its contents reach them anyway through the assistant, with a citation still opening the document it points at. Department users do not see this screen at all.",
    ...
    ["Department area (hidden)", "/departments/[id]", "Department members, their municipality admin, system admin",
     "Currently withheld from the navigation pending a decision on whether department areas belong in the product. The screen and its API still work and can be restored by changing one setting. Two tabs. Files: upload, list with uploader, date and indexing status, delete. Posts: short text posts with comments. Everything here is visible only to the department — and the assistant will quote it only for department members, even though an admin can read it here.",
